...
which(is.na(df))
```
There are also no missing values in the dataset. Although this is ideal, to ensure robustness of our model (and for project requirements), we will randomly delete data.

#### Categorical Features and Class Imbalance

There also appears to be several categorical features:
...
Here, we can see many features appear to be distributed normally, but many features are also left-skewed and right-skewed. 

Given the number of features and the number of skewed features, we will assess skewedness using the following formula:
...
* Formula acquired using online sources, https://www.itl.nist.gov/div898/handbook/eda/section3/eda35b.htm, and chatgpt.com
...
Now, we will impute the maximum outlier in the obesity feature.
```{r Impute}
...
kNN models are sensitive data distribtion (skewed data) and to multicollinearity. Therefore, we will transform the features first.
...
for (feature in skewed_features$Feature) {
  df_log[[feature]] <- log(df_log[[feature]] + 1)
...
By assessing the distribution using histograms, we can see that the log transformed features are now more normally distributed. We will proceed to deal with multicollinearity of the dataset using the previously determined correlated features.
...
Using the summary of our principle component analysis, we will choose the PCs so that it can account for roughly 90% of the variance. Given the PCA results, we will choose PC1~PC5 - as this accounts for majority of the variance.

```{r Multicollinearity2}
# Extract the first 5 PCs (multiple columns)
pc_selected <- pca_result$x[, 1:5]
...
sample_rf_eval <- evaluate_rf(train_df_rf, test_df_rf, mtry_val = 5, ntree_val = 100)
sample_rf_eval
```
...
Now, we will hyperparameterize both the number of decision trees and the number of features. To start, we will pick a standard range of between 100~600 for the number of trees, and a range betwee 2~half of the total number of features:
...
Like with kNN, we will build a few functions. First, we will build a function to create and test a ridge function given a test and training dataset.

```{r RidgeFunction}
# Get best lambda

ridge_regression <- function(train_df, test_df, a, target_index = 1){
  ...
  y_train <- as.numeric(as.factor(train_df[[target_index]])) - 1
  y_test <- as.numeric(as.factor(test_df[[target_index]])) - 1
  
  # Train model
  ridge_model <- cv.glmnet(mat_train_df[, -target_index], 
                           y_train, 
                           alpha = a, 
                           family = "binomial")
  ...
  # Predict
  pred_probs <- predict(ridge_model, s = best_lambda, newx = mat_test_df[, -target_index], type = "response")
  pred_labels <- ifelse(pred_probs > 0.5, 1, 0)
  
  # Evaluate
  ridge_eval <- evaluate_model(pred_labels, y_test)
...
```{r SampleRidge}
ridge_indics <- createDataPartition(df_ridge$Gallstone.Status, p = 0.8, list = FALSE)
train_ridge_df <- df_ridge[ridge_indics,]
test_ridge_df <- df_ridge[-ridge_indics,]

sample_ridge_eval <- ridge_regression(train_ridge_df, test_ridge_df, 1)
```

We can see that a simple hold-out method for our ridge model has an overall accuracy of `r sample_ridge_eval$accuracy` and an f1 score of `r sample_ridge_eval$f1_score`.

We can further optimize our ridge regression by optimizing the alpha parameter. The alpha parameter is used to regulate the size of the coefficients on our dataset. Given that we have substantial multicollinearity in numerous feature, optimizing alpha (as well as using ridge logistic regression) is ideal.
...
sample_ridge_tune
```
Here we can see that the function is working and provides the accuracy, f1 score, model, and the alpha value.
...
ridge_bagging <- ridge_bagging(train_ridge_df, test_ridge_df, alpha_ranges, 1)
...
ridge_prediction_list <- predict_ridge_models(ridge_bagging, ridge_ensemble_test, 1)
